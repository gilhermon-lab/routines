...
ColorOS הורג תהליכי רקע באגרסיביות. בלי שלושת אלה השגרות ייעצרו אחרי כמה שעות:

1. **פטור מאופטימיזציית סוללה** — נעשה מתוך מסך ההרשאות באפליקציה
2. **הפעלה אוטומטית** — הגדרות ← אפליקציות ← ניהול הפעלה אוטומטית ← הפעל עבור שגרות
3. **פעילות ברקע** — הגדרות ← סוללה ← שימוש בסוללה ← שגרות ← אפשר פעילות ברקע

בנוסף, בהגדרות האחרונות כדאי לכבות "אופטימיזציית שינה" אם קיימת.

## טריגר יומן

שלוש שכבות, לפי סדר עדיפות יורד:

1. **בחירה ידנית לאירוע** — גוברת על הכל
2. **מילות מפתח בכותרת** — ישיבה, ישיבת, פ"ע, פ"א. ניתנות לעריכה, וההשוואה מתעלמת מגרשיים ורווחים
3. **סינון בסיסי** — אירועים שאינם יום שלם, שלא בוטלו, ושלא נדחו

אין דרישה שהאירוע יסומן "עסוק". ישיבות רבות מסומנות "מותנה" והדרישה הזו הפילה אותן.
...
**ווידג'ט מסך הבית** — לחיצה ארוכה על מסך הבית ← Widgets ← שגרות. גרירה למסך.
לחיצה מחליפה מצב ישיבה מיד, בלי קשר ללוח הזמנים.

**אריח בהגדרות המהירות** — גלול מלמעלה, ערוך את האריחים, וגרור את "ישיבה" פנימה.

הלוגיקה של שלושת המצבים:
- **לפי הלוח** — ברירת המחדל, המצב נדלק ונכבה לפי השעות
- **דלוק ידנית** — נשאר פעיל עד שתכבה, גם מחוץ לשעות
- **כבוי ידנית** — מבטל את החלון הנוכחי בלבד, וחוזר ללוח כשהחלון נגמר

## יומן שיחות

במסך הראשי, תחת "שיחות שטופלו", מופיע מה קרה לכל שיחה: אם נדחתה או הושתקה,
באיזה מצב, והאם ההודעה אכן נשלחה. זו הדרך לוודא שההגדרות נתפסו.
